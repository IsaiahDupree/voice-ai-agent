/**
 * tests/integration/vapi-coverage.test.ts
 *
 ...
 *
 * Run: npx jest tests/integration/vapi-coverage.test.ts --testTimeout=30000
 */
...
describe('Calls', () => {
  ...
  beforeAll(async () => {
    const res = await vapiGet(`/call?assistantId=${ASSISTANT_ID}&limit=10`)
    calls = Array.isArray(res) ? res : res.results ?? []
  })
...
describe('Analytics — API-Only Resource', () => {
  test('POST /analytics returns a response', async () => {
...
describe('API-Only Resources — Existence', () => {
  test('GET /squad returns array (not 404)', async () => {
    const data = await vapiGet('/squad')
    expect(Array.isArray(data)).toBe(true)
  })

  test('GET /campaign returns array (not 404)', async () => {
    const data = await vapiGet('/campaign')
    expect(Array.isArray(data)).toBe(true)
  })

  test('GET /file returns array (not 404)', async () => {
    const data = await vapiGet('/file')
    expect(Array.isArray(data)).toBe(true)
  })

  test('GET /structured-output returns array or 404', async () => {
    const res = await fetch(`${BASE}/structured-output`, {
      headers: { Authorization: `Bearer ${VAPI_KEY}` },
    })
