...
export async function GET(request: NextRequest) {
  try {
    // Fetch map data (locations with call stats)
    const { data, error } = await supabaseAdmin
      .from('localreach_map_data')
      .select('*')
    ...
    if (error?.code === 'PGRST204' || error?.message?.includes('does not exist') || error?.message?.includes('schema cache') || error?.message?.includes('Could not find')) {
      console.warn('[LocalReach Map API] Schema error detected, returning graceful fallback:', error)
    ...
    if (error?.message?.includes('does not exist') || error?.message?.includes('schema cache') || error?.message?.includes('Could not find') || error?.message?.includes('column')) {
      console.warn('[LocalReach Map API] Schema error caught in try/catch:', error)
      return NextResponse.json({
        map: {}
      }, { status: 200 })
    }

    console.error('[LocalReach Map API] Error:', error)
    return NextResponse.json(
